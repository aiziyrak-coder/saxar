...
import { LayoutDashboard, Package, Truck, Users, Wallet, LogOut, Menu, BarChart3, Settings, Bell, ShoppingCart } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  const navItems = [
    ...
    { path: '/admin/production', icon: Package, label: 'Ishlab chiqarish' },
    { path: '/admin/orders', icon: ShoppingCart, label: 'Buyurtmalar' },
    { path: '/admin/clients', icon: Users, label: 'Mijozlar' },
    { path: '/admin/wms', icon: Package, label: 'Ombor (WMS)' },
    { path: '/admin/agents', icon: Users, label: 'Agentlar' },
    { path: '/admin/logistics', icon: Truck, label: 'Logistika' },
    { path: '/admin/finance', icon: Wallet, label: 'Moliya & Buxgalteriya' },
    { path: '/admin/reports', icon: BarChart3, label: 'Hisobotlar' },
  ...
  return (
    <div className="min-h-screen bg-emerald-50 flex relative overflow-hidden text-slate-900">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-32 top-[-10%] h-72 w-72 rounded-full bg-emerald-500/25 blur-3xl" />
      ...
      <aside className={`bg-white/70 backdrop-blur-2xl text-slate-900 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col fixed h-full z-40 shadow-[0_18px_60px_rgba(16,185,129,0.08)] border-r border-emerald-200/60`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-emerald-200/60">
          {sidebarOpen && <span className="text-xl font-bold truncate tracking-tight">Saxar ERP — Admin</span>}
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-20'}`}>
        <header className="h-16 bg-white/60 backdrop-blur-2xl border-b border-emerald-200/60 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 shadow-[0_10px_40px_rgba(16,185,129,0.10)]">
          <h1 className="text-xl font-semibold text-slate-900">
            {navItems.find(i => i.path === location.pathname)?.label || 'Boshqaruv Paneli'}
          </h1>
